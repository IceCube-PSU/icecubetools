...
from smartFormat import numFmt
...
def error(df, err_col, ref_col, left_bin_edges=None, right_bin_edges=None,
    ...
    """Take a Pandas DataFrame and extract "resolution" (median of absolute
    value) of a column as a function of another column. Arbitrary binning
    (including overlapped binning) is possible, but weighting (i.e., windows
    other than boxcar) is not supported.
    ...
    if (left_bin_edges is None) or (right_bin_edges is None):
        ...
        left_bin_edges = np.arange(
            ref_min,
            ref_max - ref_win_width+ref_step/1e3,
            ref_step
        )
        right_bin_edges = np.arange(
            ref_min + ref_win_width,
            ref_max + ref_step/1e3,
            ref_step
        )

    # Locate the 'insert' indices where the left- and right-sides of the bins
    # should lie. I.e., these indices
    left_inds = np.searchsorted(
        sorted_df[ref_col].values,
        left_bin_edges,
        side='left'
    )
    right_inds = np.searchsorted(
        sorted_df[ref_col].values,
        right_bin_edges,
        side='right'
    ) - 1
...
def resolution(df, err_col, ref_col, left_bin_edges=None, right_bin_edges=None,
    ...
    if (left_bin_edges is None) or (right_bin_edges is None):
        # Find values where left- and right-sides of bins will lie
        left_bin_edges = np.arange(
            ref_min, ref_max - ref_win_width + ref_step/1e3, ref_step
        )
        right_bin_edges = np.arange(
            ref_min + ref_win_width, ref_max + ref_step/1e3, ref_step
        )
    ...
    data = []
    if res_type is 'med':
        for n, left_ind in enumerate(left_inds):
            inds = range(left_ind, right_inds[n] + 1)
            N.append(len(inds))
        ...
        raise Exception(
            'Resolution res_type \'' + str(res_type) + '\' not implemented.'
        )
...
def uncertainty(res, N, conf=0.683, data=None, kind='root_n'):
    ...
    if kind is 'root_n':
        ...
        #    res_ax.fill_between(centers, res-1/np.sqrt(N), res+1/np.sqrt(N),
        #    alpha=0.5)
    elif kind is 'mc':
        raise Exception('Unrecognized kind: \'' + str(kind) + '\'')
    elif kind is None:
...
class ResPlot(object):
    """Make useful plots of resolutions for one dataset (or more datasets, for
    comparison purposes):
    ...
    * Number of events vs. reference parameter bin
    ...
    def __init__(self, plot_type='line',
                 ...
                 plt_fract=True, fract_fig=None, fractres_ax=None,
                 fractres_lim=None, fractnum_ax=None, fractnum_lim=None,
                 plt_num=True, num_fig=None, num_ax=None, num_lim=None,
                 **kwargs):
    ...
    def addRef(self, res, left_bin_edges=None, right_bin_edges=None, N=None,
        ...
        #self.ref['res_plot'] = plot(res, plot_type=self.plot_type,
        #                            ax=self.res_ax, **self.ref)

        #if not (N is None):
        #self.ref['num_plot'] = plot(N, plot_type=self.plot_type,
        #                            ax=self.num_ax, **self.ref)
        #else:
    ...
    def addComp(self, res, N, cut_label_tex='',
                plot_kwargs={'color': LIGHT_BLUE}, **kwargs):
        comp = {
            'res': res,
    ...
    def plotComp(self, comp):
        ...
        comp['fractres'] = None
        if self.plt_fract and comp['res'] is not None and comp != self.ref:
            if self.ref['res']:
                # Compute fractional change in resolution
                comp['fractres'] = (
                    (comp['res'] - self.ref['res']) / self.ref['res']
                )
                ...
                fractres_str = (
                    r', \,'
                    r' \langle'
                    r' \Delta(\mathrm{MAE}) / \mathrm{MAE}_0'
                    r' \rangle = '
                    + numFmt(avg_fractres)
                )
        ...
        comp['fractnum'] = None
        if self.plt_fract and comp['N'] is not None and comp != self.ref:
            comp['num_plot'] = plot(y=comp['N'], plot_type=self.plot_type,
                                    **comp)
            if self.ref['N']:
                ...
                fractnum_str = (
                    r',\,\langle\Delta N/N_0\rangle = ' + numFmt(avg_fractnum)
                )
                augment_label_strs.append(fractnum_str)

        # Update label to include avg. cut fract. eff. gain & avg cut fract.
        # gain
        ...
        if len(tex_str.strip()) == 0:
            res_label = self.err_tex
        else:
            res_label = r'$' + self.err_tex + r',\,' + tex_str + r'$'

        tex_str = comp['cut_label_tex']
        if len(tex_str.strip()) == 0:
            num_label = None
        else:
            num_label = r'$' + tex_str + r'$'

        tex_str = self.err_tex + r',\,' + comp['cut_label_tex'] + fractres_str
        if len(tex_str.strip()) == 0:
            fractres_label = None
        else:
            fractres_label = r'$' + tex_str + r'$'
        ...
        if self.plt_res and comp['res'] is not None:
            # Plot
            comp['res_plot'] = plot(y=comp['res'], plot_type=self.plot_type,
                                    ax=self.res_ax, **comp)
        ...
        # Basic number plot
        if self.plt_num and comp['N'] is not None:
            # Plot
            comp['num_plot'] = plot(y=comp['N'], plot_type=self.plot_type,
                                    ax=self.num_ax, **comp)
        ...
        if self.plt_fract and comp['fractres'] is not None:
            # Plot
            comp['fractres_plot'] = plot(y=comp['fractres'],
        ...
        if self.plt_fract and comp['fractnum'] is not None:
            # Plot
            comp['fractnum_plot'] = plot(y=comp['fractnum'],
                                         plot_type=self.plot_type,
                                         ax=self.fractnum_ax)
    ...
    def finishPlots(self):
        ...
        if self.plt_res:
            if self.x_lim is not None:
                self.res_ax.set_xlim(self.x_lim)
                if self.res_lim is not None:
                    self.res_ax.set_ylim(self.res_lim)
            self.res_leg = self.res_ax.legend(loc='best')

        if self.plt_num:
            if self.x_lim is not None:
                self.num_ax.set_xlim(self.x_lim)
                if self.num_lim is not None:
                    self.num_ax.set_ylim(self.num_lim)
            self.num_leg = self.num_ax.legend(loc='best')

        if self.plt_fract:
            if self.x_lim is not None:
                self.fractres_ax.set_xlim(self.x_lim)
            if self.fractres_lim is not None:
                self.fractres_ax.set_ylim(self.fractres_lim)
            if self.x_lim is not None:
                self.fractnum_ax.set_xlim(self.x_lim)
            if self.fractnum_lim is not None:
                self.fractnum_ax.set_ylim(self.fractnum_lim)
...
def plot(y, left_bin_edges, right_bin_edges,
    ...
    if plot_type is 'line':
        ...
        for bin_n in xrange(n_bins):
            ...
            if bin_n == 0:
                ...
                all_lines.extend(
                    ax.plot(
                        [left_edge, right_edge], [y[bin_n]]*2, **plot_kwargs
                    )
                )
    ...
    return {'fig': fig, 'ax': ax, 'legend_line': legend_line,
            'all_lines': all_lines}
